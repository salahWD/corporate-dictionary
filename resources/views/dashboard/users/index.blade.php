            ...
            <tr class="fw-normal">
              <td class="p-1">
                <a href="{{ route('user_edit', $user->id) }}">
                  <img class="ms-3 rounded-1" style="max-width: 60px" src="{{ $user->image_url() }}"
                    alt="{{ __('user image') }}">
                </a>
              </td>
              <td>
              ...
              <td><span
                  class="fw-bold px-2 py-1 text-white rounded bg-dark">{{ $user->is_admin() ? __('admin') : __('user') }}</span>
              </td>
              <td>
                <div class="btn-group">
...
@section('scripts')
  <script>
    @if (
        (Session::has('user-saved') && Session::get('user-saved')) ||
            (Session::has('user-created') && Session::get('user-created')) ||
            (Session::has('user-deleted') && Session::get('user-deleted')))
      Swal.fire({
        @if (Session::has('user-saved'))
          title: "User Saved Successfully",
        @elseif (Session::has('user-created'))
