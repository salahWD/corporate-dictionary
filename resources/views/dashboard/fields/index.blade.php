...
@section('meta')
  @php $page_title = config("app.name") . " | " . __('Manage Fields') @endphp
@endsection
  ...
  <div class="container">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
      <div class="d-block mb-4 mb-md-0">
        <x-breadcrumb page="fields" is-route=1 url="fields_manage" />
        <h2 class="h4">{{ __('All Fields') }}</h2>
      </div>
      <div class="btn-toolbar mb-2 mb-md-0">
        <a href="{{ route('field_create') }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
      ...
      <table class="table table-hover">
        <thead>
          <tr>
            ...
            <th class="border-gray-200 capitalize">{{ __('field title') }}</th>
            <th class="border-gray-200 capitalize">{{ __('parent field') }}</th>
        ...
        <tbody>
          @foreach ($fields as $field)
            ...
            <tr class="fw-normal">
              <td class="p-1">
                <a href="{{ route('field_edit', $field->id) }}">
                  <img class="ms-3 rounded-1" style="max-width: 60px" src="{{ $field->image_url() }}"
                    alt="{{ __('field image') }}">
                </a>
              </td>
              <td>
                <a href="{{ route('field_edit', $field->id) }}">{{ $field->title }}</a>
              </td>
              <td>
                <a
                  href="{{ $field->parentField?->id ? route('field_edit', $field->parentField->id) : '' }}">{{ $field->parentField?->title ?? '- - -' }}</a>
              </td>
              <td>
                <div class="btn-group">
                  ...
                  <div class="dropdown-menu py-0">
                    <a class="dropdown-item" href="{{ route('field_edit', $field->id) }}">
                      <span class="fa me-1">
                    ...
                    <hr class="my-0">
                    <form method="POST" action="{{ route('field_edit', $field->id) }}">
                      @csrf
                      @method('DELETE')
...
@section('scripts')
  <script>
    @if (
        (Session::has('field-saved') && Session::get('field-saved')) ||
            (Session::has('field-created') && Session::get('field-created')) ||
            (Session::has('field-deleted') && Session::get('field-deleted')))
      Swal.fire({
        @if (Session::has('field-saved'))
          title: "Field Saved Successfully",
        @elseif (Session::has('field-created'))
          title: "Field Created Successfully",
        @elseif (Session::has('field-deleted'))
