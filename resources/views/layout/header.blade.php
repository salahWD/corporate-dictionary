<!-- ========== { HEADER }==========  -->
<header class="fixed top-0 left-0 right-0 z-50">
  <nav class="bg-black">
    <div class="xl:container mx-auto py-2 px-3 sm:px-4 xl:px-2">
      <div class="flex justify-between">
        <a href="/" class="flex place-items-center">
          <div class="mx-w-10 text-2xl font-bold capitalize text-white flex items-center">Corporate Dictionary</div>
        </a>

        <div class="flex flex-row">
          <!-- nav menu -->
          <ul class="navbar hidden lg:flex lg:flex-row text-gray-400 text-sm items-center font-bold">
            <li class="active relative border-l border-gray-800 hover:bg-gray-900">
              <a class="block py-3 px-6 border-b-2 border-transparent" href="/">Home</a>
            </li>
            <li class="relative border-l border-gray-800 hover:bg-gray-900">
              <a class="block py-3 px-6 border-b-2 border-transparent" href="/search">Search</a>
            </li>
          </ul>

        </div>
      </div>
    </div>
  </nav>
</header><!-- end header -->
